...
class Auth extends CI_Controller {
	...
	public function register(){
		if (isset($_POST['submit1'])){
			...
			$cek  = $this->model_app->view_where('rb_reseller',array('username'=>$user))->num_rows();
			if ($cek >= 1){
				echo "<script>window.alert('Maaf, Username $user sudah dipakai oleh orang lain!');
	...
	public function login(){
		if (isset($_POST['login'])){
				...
				if ($total > 0){
					...
					if (($this->session->idp!='') and ($this->session->reseller!='')){
						$data = array(
