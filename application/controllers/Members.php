...
class Members extends CI_Controller {
	...
	function orders_report(){
        ...
        if ($session != 'konsumen'){
			...
			echo "<script>window.alert('Maaf, Anda harus login dahulu!');
	...
	function keranjang(){
		...
		if ($id_produk!=''){
			...
			if ($this->session->idp != ''){
				...
				$cek = $this->db->query("SELECT * FROM `rb_penjualan` WHERE id_pembeli='".$this->session->id_konsumen."' AND proses='4'");
				if(($cek->num_rows())>0){
					$ambil = $cek->result_array()[0];
					$this->session->set_userdata(array('idp'=>$ambil['id_penjualan']));
					redirect('members/keranjang');		
				}
			} 
			$data['title'] = 'Keranjang Belanja';
	...
	public function voucher_post(){
		...
		if(!empty($v)){
			$cek = $this->db->query("SELECT * FROM `kode_voucher` WHERE `kode_voucher`=?", array('kode_voucher'=>$v));
			if(($cek->num_rows())>0){
				$d = $cek->result_array();
				if($d[0]['jumlah_digunakan']<$d[0]['batas_jumlah_digunakan']){
					$id_produk = $d[0]['produk_id'];
					if($id_produk==0){
						$array = array(
							'status' => 2,
							'nilai'	 => $d[0]['nilai_voucher'],
							'type'	 => $d[0]['type'],
							'mes'	 => ''
						);
						...
						if(in_array($id_produk, $_POST['pid'])){
							$array = array(
								'status' => 2,
								'nilai'	 => $d[0]['nilai_voucher'],
								'type'	 => $d[0]['type'],
							...
							$array = array(
								'status' => 1,
								'nilai'	 => null,
								'mes'	 => 'Voucher tidak berlaku untuk produk ini'
							);	
						}
					}
				} else{
	...
	function acc(){
		...
		$id = $this->uri->segment(3);
		if(is_numeric($id)){
			$uid = $this->session->id_konsumen;
			$c = $this->db->query("SELECT * FROM `rb_penjualan` WHERE `id_penjualan`=?", array($id));
			if(($c->num_rows())>0){
				$g = $c->result_array()[0];
				if($uid==$g['id_pembeli']){
					$this->db->query("UPDATE `rb_penjualan` SET `proses`='3' WHERE `id_penjualan`='$id'");
	...
	function selesai_belanja(){
		...
		if (isset($_POST['submit'])){
			...
			$data1 = array(
						   ...
						   'ongkir'	=> $this->input->post('tarif_original'),
						   'proses' => is_numeric(0),
						);
			...
			if(isset($_SESSION['voucher'])){
				$voucher = array(
					'trx_id' 	=> $cekres['kode_transaksi'],
					'vcr_id' 	=> $_SESSION['voucher']['vid'],
					'konsumenid'=> $this->session->id_konsumen,
					'resellerid'=> $res['id_reseller'],
					'tgl' 		=> date('Y-m-d'),			  
				);
				$this->model_app->insert('kode_voucher_gnr', $voucher);
				$this->db->query("UPDATE `kode_voucher` SET `jumlah_digunakan`=jumlah_digunakan+1 WHERE id=?", array($_SESSION['voucher']['vid']));
			}
			if($this->input->post('dropship')){
				$atas_nama = $this->input->post('a');
				$prov = $this->input->post('b');
				...
				$this->model_app->insert('trx_dropship', $dropsip);
			}
    ...
    public function email_check(){
        if($this->input->is_ajax_request()) {
			...
			if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
				...
				$this->output->set_content_type('application/json')->set_output(json_encode(array('message' => 'Maaf, Email tidak valid,..')));
			}
		}
    }
