...
class Produk extends CI_Controller {
			...
			if ($id_produk!=''){
				if ($stok < $qty){
					...
					if ($this->session->reseller==$id_reseller){
						if ($cek >=1){
							$this->db->query("UPDATE rb_penjualan_temp SET jumlah=jumlah+$qty where session='".$this->session->idp."' AND id_produk=?", array($id_produk));
						}else{
	...
	function kurirdata(){
		...
		if(!empty($d['rajaongkir']['results'])){
			...
			$data['type']=$kurir;
			$this->load->view('posnetindo/reseller/kurirdata',$data);			
		} else{
			$data['ongkir'] = 0;
			$this->load->view('posnetindo/reseller/kurirdata',$data);	
		}
	...
	function checkouts(){
		if (isset($_POST['submit'])){
				if ($this->session->idp!=''){
					...
					echo "<script>window.alert('Terimakasih..! Order sukses');
					window.location=('".base_url()."/members/profile')</script>";
				}else{
					redirect('produk/keranjang');
				}
		}else{
			if ($this->session->id_konsumen==''){
				...
				*/
				echo "<script>window.alert('Maaf, Anda harus login terlebih dahulu!');
				window.location=('".base_url()."/auth/login')</script>";
			}
		}
	}
	...
	public function detail(){
		...
		if(preg_match("/^[A-Za-z0-9-]+$/", $ids)){
			$dat = $this->db->query("SELECT * FROM rb_produk where produk_seo=? AND id_reseller!='0'", array($ids));
			$row = $dat->row();
			$total = $dat->num_rows();
				if ($total < 1){
					redirect('main');
					...
					$data['prd'] 	= $this->model_app->view_where('kode_produk',array('produk_id'=>$row->id_produk))->row_array();
					$this->db->query("UPDATE `kode_produk` SET `viewer`=viewer+1 WHERE `produk_id`='".$row->id_produk."'");
