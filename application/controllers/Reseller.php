...
class Reseller extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			if ($total > 0){
				...
				$_SESSION['lognot'] = 'User / Password salah..';
	...
	function edit_reseller(){
		...
		if (isset($_POST['submit'])){
			...
			$array = array('gif','jpg','png','jpeg');
			if(!empty($_FILES['gg']['name'])){
				if(in_array(@end(@explode('.', $_FILES['gg']['name'])), $array)){
	...
	function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
			...
			if($cpt>6){
				...
				redirect('reseller/tambah_produk');
			} else{
    ...
    function edit_produk(){
        ...
        if (isset($_POST['submit'])){
			...
			if($cpt>6){
				$_SESSION['mes'] = 'Input Tidak Valid.';
				redirect('reseller/edit_produk/'.$_SESSION['produk_id']);
			} else{
				...
				if(!$true){
					...
					if (trim($fileName)!=''){
						$data = array('id_kategori_produk'=>$this->input->post('a'),
									'id_kategori_produk_sub'=>$this->input->post('aa'),
									'nama_produk'=>$this->input->post('b'),
									'produk_seo'=>$_SESSION['produk_seo'],
									'satuan'=>$this->input->post('c'),
									'harga_beli'=>$this->input->post('d'),
									'harga_reseller'=>$_SESSION['harga_reseller'],
									'harga_konsumen'=>$this->input->post('f'),
									'berat'=>$this->input->post('berat'),
									'gambar'=>$fileName,
									'keterangan'=>$this->input->post('ff'),
					...
					$where = array('id_produk' => $_SESSION['produk_id'],'id_reseller'=>$this->session->id_reseller);
					$this->model_app->update('rb_produk', $data, $where);
					if ($this->input->post('diskon') >= 0){
						$cek = $this->db->query("SELECT * FROM rb_produk_diskon where id_produk='".$_SESSION['produk_id']."' AND id_reseller='".$this->session->id_reseller."'");
						if ($cek->num_rows()>=1){
							$data = array('diskon'=>$this->input->post('diskon'));
							$where = array('id_produk' => $_SESSION['produk_id'],'id_reseller' => $this->session->id_reseller);
							$this->model_app->update('rb_produk_diskon', $data, $where);
						}else{
							$data = array('id_produk'=>$_SESSION['produk_id'],
										'id_reseller'=>$this->session->id_reseller,
										'diskon'=>$this->input->post('diskon'));
							$this->model_app->insert('rb_produk_diskon',$data);
						}
					...
					if ($this->input->post('stok') != ''){
						...
						$data = array('id_penjualan'=>$idp,
									'id_produk'=>$_SESSION['produk_id'],
									'jumlah'=>$this->input->post('stok'),
									'harga_jual'=>$_SESSION['harga_reseller'],
									'satuan'=>$this->input->post('c'));
						$this->model_app->insert('rb_penjualan_detail',$data);
					}
					unset($_SESSION['produk_seo']);
					unset($_SESSION['produk_id']);
					unset($_SESSION['harga_reseller']);
					redirect('reseller/produk');					
				}
	...
	function edit_pembelian(){
		...
		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_pembelian/view_pembelian_edit',$data);
	...
	function penjualan(){
		...
		$this->session->unset_userdata('idp');
		$id = $this->session->id_reseller;
		$data['record'] = $this->model_reseller->penjualan_list_konsumen_3($id,'reseller');
	...
	function proses_penjualan(){
		cek_session_reseller();
			...
			$this->model_app->update('rb_penjualan', $data, $where);
			redirect($this->uri->segment(1).'/orderan');
	}
	...
	function orderan(){
		...
		$this->session->unset_userdata('idp');
		$id = $this->session->id_reseller;
		$data['record'] = $this->model_reseller->penjualan_list_konsumen_2($id,'reseller');
		$this->db->query("UPDATE rb_penjualan a JOIN rb_konfirmasi_pembayaran_konsumen b ON b.id_penjualan = a.id_penjualan SET b.status_read='2' WHERE a.id_penjual='".$id."'");
		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_konsumen/view_orderan',$data);
	}
