...
class Model_reseller extends CI_model{
    ...
    function penjualan_list_konsumen($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.status_penjual='$level' AND a.id_penjual='$id' AND a.proses!='4' ORDER BY a.id_penjualan DESC");
    }

    function penjualan_list_konsumen_3($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.status_penjual='$level' AND a.id_penjual='$id' AND a.proses='3' ORDER BY a.id_penjualan DESC");
    }

    function penjualan_list_konsumen_2($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen JOIN rb_konfirmasi_pembayaran_konsumen c ON c.id_penjualan = a.id_penjualan where a.status_penjual='$level' AND a.id_penjual='$id' AND a.proses!='4' AND a.proses!='3' AND c.konf='2' ORDER BY a.id_penjualan DESC");
    }
    ...
    function orders_report($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_penjual=b.id_reseller where a.status_penjual='$level' AND a.id_pembeli=? AND a.proses!='4' ORDER BY a.id_penjualan DESC", array($id));
    }
    ...
    function penjualan_list_konsumen_top($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen JOIN rb_konfirmasi_pembayaran_konsumen c ON c.id_penjualan = a.id_penjualan where a.status_penjual='$level' AND a.id_penjual='$id' AND c.konf='2' AND c.status_read='1' ORDER BY a.id_penjualan DESC");
    }
    ...
    function produk_perkategori($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND a.id_kategori_produk='$id_kategori_produk' ORDER BY a.id_produk DESC LIMIT $limit");
    }
