            <div class="col-xs-12">  
              <div class="box">
                ...
                <?php 
                  ...
                  echo form_open_multipart($this->router->fetch_class().'/edit_pembelian',$attributes); 
                ?>
                  ...
                  <tbody>
                      ...
                      <?php 
                        foreach ($supplier as $r){
                          if ($r['id_supplier']==$rows['id_supplier']){
                            echo "<option value='$r[id_supplier]' selected>".stripslashes($r[nama_supplier])."</option>";
                          } else{
                            echo "<option value='$r[id_supplier]'>".stripslashes($r[nama_supplier])."</option>";
                          }
                        }
                        ?>
                        </select></td>
                    ...
                    <tr>
                      ...
                      <td><input type='text' class='form-control' value='<?php echo "$rows[waktu_beli]"; ?>' name='c'></td>
                    </tr>
                  </tbody>
                  </table>
                  <input class='btn btn-primary btn-sm' type="submit" name='submit1' value='Simpan Data'>
                  <a class='btn btn-default btn-sm' href='<?php echo base_url().$this->router->fetch_class(); ?>/pembelian'>Selesai / Kembali</a>
                  <hr>
                    ...
                    <?php 
                        echo "<tr>
                                ...
                                <td><button type='submit' name='submit' class='btn btn-success  btn-xs'><span class='glyphicon glyphicon-ok' aria-hidden='true'></span></button>
                                    <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url().$this->router->fetch_class()."/edit_pembelian/".$this->uri->segment(3)."'><span class='glyphicon glyphicon-remove'></span></a>
                                </td>
                              </tr>";
                      ?>
                    <tbody>
                  <?php 
                    ...
                    echo "<tr><td>$no</td>
                              <td>".stripslashes($row[nama_produk])."</td>
                              <td>Rp ".rupiah($row['harga_pesan'])."</td>
                              <td>$row[jumlah_pesan]</td>
                              <td>$row[satuan]</td>
                              <td>Rp ".rupiah($row['harga_pesan']*$row['jumlah_pesan'])."</td>
                              <td>
                                <a class='btn btn-warning btn-xs' title='Edit Data' href='".base_url().$this->router->fetch_class()."/edit_pembelian/".$this->uri->segment(3)."/$row[id_pembelian_detail]'><span class='glyphicon glyphicon-edit'></span></a>
                                <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url().$this->router->fetch_class()."/delete_pembelian_detail/".$this->uri->segment(3)."/$row[id_pembelian_detail]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                              </td>
                          </tr>";
                      $no++;
                    }
