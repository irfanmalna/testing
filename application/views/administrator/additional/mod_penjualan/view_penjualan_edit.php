            <div class="col-xs-12">  
              <div class="box">
                ...
                <?php 
                  ...
                  echo form_open_multipart($this->router->fetch_class().'/edit_penjualan',$attributes); 
                ?>
                  <table class='table table-condensed table-bordered'>
                  <tbody>
                    <tr>
                      <th width='140px' scope='row'>Kode penjualan</th>  
                      <td><input type='text' class='form-control' value='<?php echo "$rows[kode_transaksi]"; ?>' name='a'></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td><input type='text' class='form-control' value='<?php echo "$rows[waktu_transaksi]"; ?>' name='c'></td>
                    </tr>
                  </tbody>
                  </table>
                  <input class='btn btn-primary btn-sm' type="submit" name='submit1' value='Simpan Data'>
                  <a class='btn btn-default btn-sm' href='<?php echo base_url().$this->router->fetch_class(); ?>/penjualan'>Selesai / Kembali</a>
                  <hr>
                    ...
                    <?php 
                        echo "<tr>
                                ...
                                <td><button type='submit' name='submit' class='btn btn-success  btn-xs'><span class='glyphicon glyphicon-ok' aria-hidden='true'></span></button>
                                    <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url().$this->router->fetch_class()."/edit_penjualan/".$this->uri->segment(3)."'><span class='glyphicon glyphicon-remove'></span></a>
                                </td>
                              </tr>";
                      ?>
                    <tbody>
                  <?php 
                    ...
                    echo "<tr><td>$no</td>
                              ...
                              <td>
                                <a class='btn btn-warning btn-xs' title='Edit Data' href='".base_url().$this->router->fetch_class()."/edit_penjualan/".$this->uri->segment(3)."/$row[id_penjualan_detail]'><span class='glyphicon glyphicon-edit'></span></a>
                                <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url().$this->router->fetch_class()."/delete_penjualan_detail/".$this->uri->segment(3)."/$row[id_penjualan_detail]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                              </td>
                          </tr>";
                      $no++;
                    }
