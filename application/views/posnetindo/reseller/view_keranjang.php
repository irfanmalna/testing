...
?>
      <table class="table table-striped">
        ...
        <?php 
          ...
          $total = $this->db->query("SELECT sum((a.harga_jual*a.jumlah)-a.diskon) as totals, sum(b.berat*a.jumlah) as total_berat FROM `rb_penjualan_temp` a JOIN rb_produk b ON a.id_produk=b.id_produk where a.session='".$this->session->idp."'")->row_array();
          echo "<tr>
                  <td colspan='3'><b>Total Harga</b></td>
                  <td><b>Rp ".rupiah($total['totals'])."</b></td>
                  <td></td>
                </tr>
                <tr>
...
<div class="col-sm-4 colom44">
  <?php $res = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM rb_reseller a JOIN rb_kota b ON a.kota_id=b.kota_id JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id
